...
//	upstep_last_num = upstep_num;
	...
	if(bullet_remain != My_Judge.org_info->projectile_allowance.projectile_allowance_42mm)
	{
		dynamic_ui_info[BULLET_NUM].ui_config.int_num = (int16_t)My_Judge.org_info->projectile_allowance.projectile_allowance_42mm;
		Enqueue_Ui_For_Sending(&dynamic_ui_info[BULLET_NUM]);
	}
	
	bullet_remain =  My_Judge.org_info->projectile_allowance.projectile_allowance_42mm;
...
//		dynamic_ui_info[CAP_LINE].ui_config.end_y = Client_mid_position_y + 320;
	...
	theta_err_now = -Chassis.Leg_Unit[R_Leg]->Straight->info->thetal_now;//右视图
	...
	test_chas_angle = -(YAW_MOTOR_ANGLE_MIDDLE - Yaw_Motor.rx_info->motor_angle);
...
//	test_chas_angle = (test_chas_angle / 4096.f) * PI;
	...
	//车体速度更新
	static float speed_result = 0;
	static float speed_last = 0.f,speed_now = 0.f;
	
	speed_now = speed_result;
	...
	else if(Balance.Flag->Fly_Flag == true)//中腿长标志位
	{
		length_mode = 2;
	}
	else
...
{	
	if(gimbal.base_info.yaw_mec_angle_target == YAW_MOTOR_ANGLE_MIDDLE)
	{
	  test_l = constrain(Link[L_Leg].info->length->l0, MIN_LEG_LENGTH, MAX_LEG_LENGTH) - MIN_LEG_LENGTH;
	  test_r = constrain(Link[R_Leg].info->length->l0, MIN_LEG_LENGTH, MAX_LEG_LENGTH) - MIN_LEG_LENGTH;
	}
	else if(gimbal.base_info.yaw_mec_angle_target ==  YAW_MOTOR_ANGLE_MIDDLE - PI)
	{
		test_r = constrain(Link[R_Leg].info->length->l0, MIN_LEG_LENGTH, MAX_LEG_LENGTH) - MIN_LEG_LENGTH;
	  test_l = constrain(Link[L_Leg].info->length->l0, MIN_LEG_LENGTH, MAX_LEG_LENGTH) - MIN_LEG_LENGTH;
	}
	...
	float gragh_range = fabsf((float)const_ui_info[HIGH_LINE].ui_config.start_y - (float)const_ui_info[MINIMUM_LINE].ui_config.start_y);
