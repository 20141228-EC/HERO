...
// uint8_t is_com_vision:1;  // 位3：普通自瞄模式
// uint8_t outpost_mode:1  ;// 位4：只锁前哨模式
// uint8_t engineer_mode:1 ;// 位5：只锁工程模式
// uint8_t lob_mode:1  ; // 位6：只吊射模式
// uint32_t reserved :25 ; // 位7-31：可扩展
// } bit; // 按位访问的子结构
// } flag_union; 
 ...
 * @file vision_protocol.h
 * @author Isaac
 * @brief Vision protocol definitions.
 * @version 0.1
 * @date 2023-11-21
 */
...
#define VISION_OFFLINE_CNT_MAX (80)
...
typedef struct __attribute__((packed))
{
    ...
    __packed union {
        ...
        __packed struct {
            ...
            uint8_t outpost_mode : 1;
            uint8_t engineer_mode : 1;
            uint8_t lob_mode : 1;
...
typedef struct __attribute__((packed))
{
    ...
    __packed union {
        uint32_t all_flags;
        __packed struct {
            uint8_t is_find_target : 1;
            uint8_t is_keep_shooting : 1;
            uint8_t is_enable_shootting : 1;
            uint8_t detect_num : 4;
            uint32_t reserved : 25;
        } bit;
    } flag_union;
...
typedef struct __attribute__((packed))
{
    dev_work_state_t tx_state;
    dev_work_state_t rx_state;
    uint32_t send_time;
    uint32_t rx_tick;
    uint8_t offline_cnt;
