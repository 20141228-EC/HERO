...
//		vision.EtoV->pitch = gimbal.base_info.pitch_motor_angle / PITCH_MOTOR_HALF_ENCODER * 180.f;    //360
//	}
//	else
//	{
//		vision.EtoV->yaw = Board_Rx_Info.yaw_mec_imu ;/// PI * 180.f;
 ...
 * @file vision_protocol.c
 * @author Isaac
 * @brief Vision protocol implementation.
 * @version 0.1
 * @date 2023-11-21
 */
...
#define VISION_RX_FRAME_SIZE ((uint32_t)sizeof(VisionToElectricalFrame))
#define VISION_RX_STREAM_BUF_SIZE (VISION_RX_FRAME_SIZE * 4U)

static void Vision_TxTime_Calculating(void);
static void Append_Vision_Timing_Buff(uint32_t *vision_timing_buff, uint8_t size, uint16_t delay);
static void Vision_DataRxFrame(const uint8_t *rxBuf);
...
uint8_t vision_txBuf[80];
static uint8_t vision_rx_stream_buf[VISION_RX_STREAM_BUF_SIZE];
static uint32_t vision_rx_stream_len = 0U;
...
static void Vision_TxTime_Calculating(void)
{
    ...
    vision.status->send_time = tick - last_tick;
    last_tick = tick;
}

static void Vision_DataRxFrame(const uint8_t *rxBuf)
{
    if (rxBuf == NULL || rxBuf[0] != 0xA5U) {
        return;
    }

    if (Verify_CRC8_Check_Sum((uint8_t *)rxBuf, 6) != true) {
        return;
    }

    if (Verify_CRC16_Check_Sum((uint8_t *)rxBuf, sizeof(VisionToElectricalFrame)) != true) {
        return;
    }

    memcpy(&vision_rx_info, rxBuf, sizeof(VisionToElectricalFrame));
    vision.status->offline_cnt = 0;
    vision.status->rx_tick = HAL_GetTick();

    if (vision.VtoE->flag_union.bit.is_enable_shootting == 1U) {
        Append_Vision_Timing_Buff(
            (uint32_t *)vision.timestamp_info->vision_shoot_timing,
            (uint8_t)(sizeof(vision.timestamp_info->vision_shoot_timing) /
                      sizeof(vision_timestamp_info.vision_shoot_timing[0])),
            vision.VtoE->timing);
    }
}

void Vision_DataRxBytes(const uint8_t *rxBuf, uint32_t len)
{
    uint32_t i;

    if (rxBuf == NULL || len == 0U) {
        return;
    }

    for (i = 0U; i < len; ++i) {
        if (vision_rx_stream_len >= VISION_RX_STREAM_BUF_SIZE) {
            memmove(
                vision_rx_stream_buf,
                vision_rx_stream_buf + 1,
                VISION_RX_STREAM_BUF_SIZE - 1U);
            vision_rx_stream_len = VISION_RX_STREAM_BUF_SIZE - 1U;
        }
        vision_rx_stream_buf[vision_rx_stream_len++] = rxBuf[i];
    }

    while (vision_rx_stream_len >= VISION_RX_FRAME_SIZE) {
        uint32_t start = 0U;
        uint8_t found_valid_frame = 0U;

        while (start < vision_rx_stream_len && vision_rx_stream_buf[start] != 0xA5U) {
            ++start;
        }
        ...
        if (start > 0U) {
            memmove(
                vision_rx_stream_buf,
                vision_rx_stream_buf + start,
                vision_rx_stream_len - start);
            vision_rx_stream_len -= start;
        }

        if (vision_rx_stream_len < VISION_RX_FRAME_SIZE) {
            break;
        }

        if (Verify_CRC8_Check_Sum(vision_rx_stream_buf, 6) == true &&
            Verify_CRC16_Check_Sum(vision_rx_stream_buf, VISION_RX_FRAME_SIZE) == true) {
            Vision_DataRxFrame(vision_rx_stream_buf);
            memmove(
                vision_rx_stream_buf,
                vision_rx_stream_buf + VISION_RX_FRAME_SIZE,
                vision_rx_stream_len - VISION_RX_FRAME_SIZE);
            vision_rx_stream_len -= VISION_RX_FRAME_SIZE;
            found_valid_frame = 1U;
        }

        if (found_valid_frame == 0U) {
            memmove(
                vision_rx_stream_buf,
                vision_rx_stream_buf + 1,
                vision_rx_stream_len - 1U);
            vision_rx_stream_len -= 1U;
        }
    }
}
...
void Vision_DataTx(UART_HandleTypeDef *huart)
{
    ...
    memcpy(vision_txBuf, &vision_tx_info, sizeof(ElectricalToVisionFrame));
    Append_CRC8_Check_Sum(vision_txBuf, 6);
    Append_CRC16_Check_Sum(vision_txBuf, sizeof(ElectricalToVisionFrame));

    if (!CDC_IsConfigured_FS()) {
        vision.status->tx_state = DEV_OFFLINE;
        return;
    }
...
void Vision_Board_Update(void)
{
    ...
    if (gimbal.gimbal_ctrl_mode.gimbal_mode != 1) {
        vision.EtoV->yaw = Board_Rx_Info.yaw_mec_imu / PI * 180.0f;
        vision.EtoV->pitch = gimbal.base_info.pitch_motor_angle / PITCH_MOTOR_HALF_ENCODER * 180.0f;
    } else {
        vision.EtoV->yaw = Board_Rx_Info.yaw_mec_imu;
        vision.EtoV->pitch = gimbal.base_info.pitch_imu_angle;
    }
...
{
    vision.status->offline_cnt++;
    if (vision.status->offline_cnt > vision.status->offline_cnt_max) {
        vision.status->offline_cnt = vision.status->offline_cnt_max;
        vision.status->rx_state = DEV_OFFLINE;
    } else if (vision.status->rx_state == DEV_OFFLINE) {
        vision.status->rx_state = DEV_ONLINE;
    }
...
static void Append_Vision_Timing_Buff(uint32_t *vision_timing_buff, uint8_t size, uint16_t delay)
{
    ...
    for (i = 1U; i < size; i++) {
        vision_timing_buff[i - 1U] = vision_timing_buff[i];
    }

    vision_timing_buff[size - 1U] = delay + HAL_GetTick();
}
...
void Vision_led_work(void)
{
    if (vision.status->rx_state == DEV_OFFLINE) {
        led.colour = LED_colour_red;
        led.state = LED_ON;
    } else if (vision.VtoE->flag_union.bit.is_find_target == 1U) {
        led.colour = LED_colour_green;
        led.state = LED_ON;
    } else if (vision.status->rx_state == DEV_ONLINE) {
        led.colour = LED_colour_green;
