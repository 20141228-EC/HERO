...
*/
HAL_StatusTypeDef CAN_Send_With_int16_to_uint8(CAN_HandleTypeDef *hcan,uint32_t stdId, int16_t *dat)
{
	uint8_t data[8];
	uint32_t txMailBox;
	
	int16_to_uint8(data,dat);

	HAL_CAN_TxHeadeInit(stdId);//配置ID
  if(HAL_CAN_AddTxMessage(hcan,&CAN_TxHeadeType,data,&txMailBox)!=HAL_OK)
	{
		return HAL_ERROR;
	}
	return HAL_OK;
