...
static void rc_sensor_check(rc_sensor_t *rc_sen)
{
	...
	rc_sen->info->sw1.value=rc_sen->info->s1;
	rc_sen->info->sw2.value=rc_sen->info->s2;
	/*拨轮产生阶跃信号*/
	...
	if((rc_info->thumbwheel.value == 0) && (thumbwheel_record != 0))
	{
		...
		thumbwheel_record = 0;
        	...
        	/* 左拨杆判断 */
          if(rc_sen->info->sw1.value!= rc_sen->info->sw1.value_last)
          {
            switch(rc_sen->info->sw1.value)
            {
              case 1:
                rc_sen->info->sw1.status = up_R;
                break;
              case 3:
                rc_sen->info->sw1.status = mid_R;
                break;
              case 2:
                rc_sen->info->sw1.status = down_R;
                break;
              default:
                break;
            ...
            rc_sen->info->sw1.status = keep_R;
          
          /* 右拨杆判断 */
           if(rc_sen->info->sw2.value!= rc_sen->info->sw2.value_last)
          {
            switch(rc_sen->info->sw2.value)
            {
              case 1:
                rc_sen->info->sw2.status = up_R;
                break;
              case 3:
                rc_sen->info->sw2.status = mid_R;
                break;
              case 2:
                rc_sen->info->sw2.status = down_R;
                break;
              default:
                break;
            }
	...
	rc_sen->info->sw2.value_last = rc_sen->info->sw2.value;
	rc_sen->info->sw1.value_last = rc_sen->info->sw1.value;
}
