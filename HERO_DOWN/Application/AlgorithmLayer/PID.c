...
float  all_pid_calc (pid_ctrl_t *out,pid_ctrl_t *inn,float target,float mea_out,float mea_in,float inner_kp,uint8_t err_cal_mode)
{
	if(inn == NULL)return 0;  //没有内环，为0
	 ...
	 else if(out == NULL&&inn!=NULL)  //只有速度环,不用处理半圈什么的
	{
		inn->target=target;
		inn->measure=mea_in;
...
{
	 pid_ctrl_t *out	  = NULL;
	 pid_ctrl_t *inn	  = speed_pid;
	 float target   	  = speed_target;
	 float mea_out       = NULL;
	 float mea_in        = motor_speed;
	 float inner_kp      = 0;
	 uint8_t err_cal_mode= 0;
	 return all_pid_calc (out,inn,target,mea_out,mea_in,inner_kp,err_cal_mode);
