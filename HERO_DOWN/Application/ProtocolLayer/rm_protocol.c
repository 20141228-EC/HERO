...
{
    rm_motor_info_t *motor_info = rm_motor->info;  //将建立的结构体指针定向到传入的rm_motor->info
...
void rm_motor_check(rm_motor_t *motor)
{
	...
	/* 过零点 */
	if(abs(err) > 4095)  //零点设置大一点或者小一点可能也没关系
	{
		/* 0↓ -> 8191 */
		if(err >= 0)  
			motor_info->angle_sum += -8191 + err;
		/* 8191↑ -> 0 */
		else  				//也就是0（now） - 8191（pre），得到err是-8191，但其实只是走了一点，不用搞这么大，所以要平衡一下
			motor_info->angle_sum += 8191 + err;
	}
	/* 未过零点 */
	else
