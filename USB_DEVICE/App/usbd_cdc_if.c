...
//  */
////USBD_CDC_HandleTypeDef hcdc_test;
//uint8_t CDC_Transmit_FS(uint8_t* Buf, uint16_t Len)
//{
//  uint8_t result = USBD_OK;
//  /* USER CODE BEGIN 7 */
//  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*)hUsbDeviceFS.pClassData;
////	hcdc_test = (USBD_CDC_HandleTypeDef)hUsbDeviceFS.pClassData;
////if(hcdc->TxState == 1) {
////  hcdc->TxState = 0;
////}
//  if (hcdc->TxState != 0){
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  {
    Vision_DataRxBytes(Buf, *Len);
  }

  USBD_CDC_SetRxBuffer(&hUsbDeviceFS, (Buf != NULL) ? &Buf[0] : UserRxBufferFS);
  USBD_CDC_ReceivePacket(&hUsbDeviceFS);
  return (USBD_OK);
  /* USER CODE END 6 */
}
