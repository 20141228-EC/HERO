...
{
	angle = angle % 8192;
	motor->motor_all_pid.angle_pid.angle.target=angle;
...
{
	motor->motor_all_pid.position_pid.angle.target=angle;
...
float rm_motor_anglesum_pid_calc(rm_motor_t *motor, int32_t angle)
{
	...
	//角度环
	single_pid_ctrl(&motor->motor_all_pid.position_pid.angle);
	motor->motor_all_pid.position_pid.speed.target=motor->motor_all_pid.position_pid.angle.out;
	motor->motor_all_pid.position_pid.speed.err =motor->motor_all_pid.position_pid.speed.target - motor->info->speed;
	//速度环
	single_pid_ctrl(&motor->motor_all_pid.position_pid.speed);
...
int8_t Motor_DetectStuck_userdef(rm_motor_t *motor,uint16_t speed,uint16_t current,uint16_t time_ms,uint8_t force_direction)
{
	...
	else if(abs(motor->info->speed)<=speed && abs(motor->info->current) > current&&force_direction>0&&motor->info->current>0)//任意方向
	{
			motor->cnt++;
	}
	else if(abs(motor->info->speed)<=speed && abs(motor->info->current) > current&&force_direction<0&&motor->info->current<0)//任意方向
	...
	if(motor->cnt >= time_ms)//达到堵转条件并且计时够
	{
		motor->cnt = time_ms;
		return 1;
	}
