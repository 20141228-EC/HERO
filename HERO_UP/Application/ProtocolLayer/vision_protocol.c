...
Vison_Rx_Info_t vision_rx_info;
...
void Vision_DataRx(uint8_t *rxBuf)
{
	...
	if(rxBuf[0] == 0xA5)
	{
		...
		{
			/* 帧尾CRC16校验 */
			if(Verify_CRC16_Check_Sum(rxBuf, sizeof(Vison_Rx_Info_t)) == true)
			{
				memcpy(&vision_rx_info, rxBuf, sizeof(Vison_Rx_Info_t));
				vision.status->offline_cnt = 0;
				vision.status->rx_tick = HAL_GetTick();//记录接受到信息时的时间，好像没用到
				//添加发射时间戳
				if(vision.rx_info->is_shoot_enable == 1)
				{
...
void Vision_DataTx(UART_HandleTypeDef *huart)
{
	...
	if(CDC_Transmit_FS(vision_txBuf,sizeof(Vision_Tx_Info_t)) == USBD_OK)//串口发送
	{
...
void Vision_led_work(void)
{
	...
	else if(vision.rx_info->is_find_target==1)
	{
