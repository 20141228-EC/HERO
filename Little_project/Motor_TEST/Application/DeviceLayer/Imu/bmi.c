...
uint8_t BMI_Get_EulerAngle(float *pitch,float *roll,float *yaw,\
                           float *pitch_rate,float *roll_rate,float *yaw_rate,\
						   ...
						   float *ax,float *ay,float *az)
{
    ...
    /* 角度制to弧度制 */
	*roll_rate  = bmi.gx / (double)0.017453;
	*pitch_rate = bmi.gy / (double)0.017453;
	*yaw_rate   = bmi.gz / (double)0.017453;
	...
	if(bmi.ax * bmi.ay * bmi.az != 0)
	{
        ...
        /* 误差补偿，Kp越大，越信任加速度计 */
        bmi.gx = bmi.gx + bmi.ex*bmi.Kp*kx;
        bmi.gy = bmi.gy + bmi.ey*bmi.Kp*ky;
        bmi.gz = bmi.gz + bmi.ez*bmi.Kp*kz;
	}
...
void array_to_value(float *a,float *b,float *c,float array[3])
{
